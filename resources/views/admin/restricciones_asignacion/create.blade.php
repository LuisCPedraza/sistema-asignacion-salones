...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold mb-6">Crear Restricción de Asignación</h1>
    <form method="POST" action="{{ route('admin.restricciones_asignacion.store') }}">
        @csrf
        <div class="mb-4">
            ...
            <select name="recurso_type" id="recurso_type" class="border rounded px-2 py-1 w-full @error('recurso_type') border-red-500 @enderror" required onchange="toggleRecurso()">
                <option value="" disabled {{ old('recurso_type') ? '' : 'selected' }}>Selecciona tipo</option>
                <option value="salon" {{ old('recurso_type') == 'salon' ? 'selected' : '' }}>Salón</option>
                <option value="profesor" {{ old('recurso_type') == 'profesor' ? 'selected' : '' }}>Profesor</option>
            </select>
        ...
        <div class="mb-4" id="recurso_id_div">
            ...
            <select name="recurso_id" id="recurso_id" class="border rounded px-2 py-1 w-full @error('recurso_id') border-red-500 @enderror" required>
                <option value="" disabled {{ old('recurso_id') ? '' : 'selected' }}>Selecciona recurso</option>
                @foreach ($salones as $salon)
                    <option value="{{ $salon->id }}" {{ old('recurso_id') == $salon->id ? 'selected' : '' }}>{{ $salon->codigo }}</option>
                @endforeach
                @foreach ($profesores as $profesor)
                    <option value="{{ $profesor->id }}" {{ old('recurso_id') == $profesor->id ? 'selected' : '' }}>{{ $profesor->user->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="tipo_restriccion" id="tipo_restriccion" class="border rounded px-2 py-1 w-full @error('tipo_restriccion') border-red-500 @enderror" required>
                <option value="" disabled {{ old('tipo_restriccion') ? '' : 'selected' }}>Selecciona tipo</option>
                <option value="horario" {{ old('tipo_restriccion') == 'horario' ? 'selected' : '' }}>Horario</option>
                <option value="capacidad" {{ old('tipo_restriccion') == 'capacidad' ? 'selected' : '' }}>Capacidad</option>
                <option value="especial" {{ old('tipo_restriccion') == 'especial' ? 'selected' : '' }}>Especial</option>
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="valor" id="valor" rows="3" class="border rounded px-2 py-1 w-full @error('valor') border-red-500 @enderror" placeholder='{"dias": ["lun", "mie"], "max_cap": 30}'>{{ old('valor') }}</textarea>
        ...
        <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">Crear Restricción</button>
...
function toggleRecurso() {
    ...
    if (type === 'salon') {
        @foreach ($salones as $salon)
            select.innerHTML += '<option value="{{ $salon->id }}">{{ $salon->codigo }}</option>';
        @endforeach
    } else if (type === 'profesor') {
        @foreach ($profesores as $profesor)
            select.innerHTML += '<option value="{{ $profesor->id }}">{{ $profesor->user->name }}</option>';
        @endforeach
    }
}
</script>
