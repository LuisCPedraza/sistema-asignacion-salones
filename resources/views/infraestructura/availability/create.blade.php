...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>➕ Agregar Disponibilidad para {{ $classroom->name }} ({{ $classroom->code }})</h1>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('infraestructura.classrooms.availabilities.store', $classroom) }}">
                @csrf
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select name="availability_type" class="form-select @error('availability_type') is-invalid @enderror" required>
                                <option value="regular" {{ old('availability_type') == 'regular' ? 'selected' : '' }}>Regular</option>
                                <option value="maintenance" {{ old('availability_type') == 'maintenance' ? 'selected' : '' }}>Mantenimiento</option>
                                <option value="reserved" {{ old('availability_type') == 'reserved' ? 'selected' : '' }}>Reservado</option>
                                <option value="special_event" {{ old('availability_type') == 'special_event' ? 'selected' : '' }}>Evento Especial</option>
                            </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" 
                        rows="3" placeholder="Ej: En mantenimiento, reservado para evento, etc.">{{ old('notes') }}</textarea>
